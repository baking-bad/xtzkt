...
public class AddressHashBinder : IModelBinder
{
    public Task BindModelAsync(ModelBindingContext bindingContext)
    {
        ...
        if (!bindingContext.TryGetAddressHash($"{param}.ne", ref hasValue, out var ne))
            return Task.CompletedTask;

        if (!bindingContext.TryGetAddressHashList($"{param}.in", ref hasValue, out var @in))
            return Task.CompletedTask;

        if (!bindingContext.TryGetAddressHashList($"{param}.ni", ref hasValue, out var ni))
            return Task.CompletedTask;
        ...
        bindingContext.Result = ModelBindingResult.Success(!hasValue ? null : new AddressHashParameter
        {
            Eq = _eq,
            Ne = ne,
            In = @in,
