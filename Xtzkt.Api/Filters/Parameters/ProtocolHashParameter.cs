...
[ModelBinder(BinderType = typeof(ProtocolHashBinder))]
public class ProtocolHashParameter : INormalizable
{
    /// <summary>
    /// **Equal** mode (default mode, so `param.eq=value` is the same as `param=value`).
    /// Returns items where 'param' is equal to 'value'.
    ///
    /// Example: `?hash=P...` or `?hash=0x...`.
    /// </summary>
    ...
    ///
    /// Example: `?hash.ni=P...,0x...`.
    /// </summary>
    public List<string>? Ni { get; set; }

    public static implicit operator ProtocolHashParameter(string value) => new() { Eq = value };
