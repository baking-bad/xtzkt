...
{
    protected virtual async Task<Blueprint> GetBlueprint(int level)
    {
    ...
    protected virtual void CacheDelayedTransactions(JsonElement json, int level)
    {
    ...
    protected virtual async Task<DelayedOperation> ResolveDelayedTransaction(string hash, int level)
    {
    ...
    protected virtual DelayedFaDeposit ParseDelayedFaDeposit(byte[] hash, byte[] bytes)
    {
        var stream = new RlpStream(bytes);
        if (stream.Read() is not RlpList rlp || stream.CanRead)
    ...
    protected virtual DelayedEvmTransaction ParseDelayedEvmTransaction(byte[] hash)
    {
    ...
    protected virtual DelayedMichelsonOperation ParseDelayedMichelsonOperation(byte[] hash)
    {
