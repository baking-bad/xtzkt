...
public partial class ProtoHelpers
{
            ...
            switch (candidate)
            {
                case EvmOperation op:
                    ...
                    if (delayedDeposit is DelayedFaDeposit faDeposit)
                    {
                        ...
                        var operation = new EvmDeposit { Deposit = delayedDeposit, FeederCall = op };

                        if (faDeposit.Proxy != null)
                            operation.Internals.Add(SynthesizeProxyCall(op, faDeposit));
    ...
    protected virtual EvmInternalOperation SynthesizeProxyCall(EvmOperation feederCall, DelayedFaDeposit faDeposit)
    {
        var succeeded = true;
