...
namespace Xtzkt.Indexers.TezosX.Protocols.Proto08
{
    class BigMapCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        ...
        readonly List<(IBigmapOperation op, XMichelsonContract contract, BigMapDiff diff)> Diffs = [];
        readonly Dictionary<int, int> TempPtrs = new(7);
        int TempPtr = 0;

        public virtual void Append(IBigmapOperation op, XMichelsonContract contract, IEnumerable<BigMapDiff> diffs)
        {
        ...
        public virtual async Task Apply()
        {
            ...
            Context.Block.Events |= XBlockEvents.Bigmaps;
        ...
        protected virtual BigMapTag GetTags(XMichelsonContract contract, TreeView node) => BigMaps.GetTags(contract, node);

        public virtual async Task Revert(XBlock block)
        {
            if (block.Events.HasFlag(XBlockEvents.Bigmaps))
            {
