...
{
    public static async Task<List<RegisterConstantOperation>> Find(XtzktContext db, int chainId, IEnumerable<IMicheline> nodes)
    {
        ...
        while (true)
        {
            ...
            var constants = await db.RegisterConstantOps
                .Where(x => x.ChainId == chainId && x.Address != null && pendingConstants.Contains(x.Address))
                .ToListAsync();
