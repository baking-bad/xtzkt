...
public static class BigMaps
{
    public static BigMapTag GetTags(L1Contract contract, TreeView bigmap)
    {
        ...
        if (IsPersistent(bigmap))
        {
            ...
            else if (contract.Kind == L1ContractKind.Asset && schema.Field == "token_metadata")
            {
        ...
        if (contract.Kind == L1ContractKind.Asset && schema.Field == "ledger")
        {
    ...
    public static BigMapTag GetTags(XMichelsonContract contract, TreeView bigmap)
    {
        var schema = (bigmap.Schema as BigMapSchema)!;
        var tags = BigMapTag.None;
        ...
        if (IsPersistent(bigmap))
        {
            ...
            else if (contract.Kind == XContractKind.Asset && schema.Field == "token_metadata")
            {
                if (schema.Key is NatSchema &&
                    schema.Value is PairSchema pair &&
                        pair.Left is NatSchema &&
        ...
        if (contract.Kind == XContractKind.Asset && schema.Field == "ledger")
        {
            tags |= GetLedgerType(schema);
        }
