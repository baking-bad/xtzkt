...
public class ProposalsCache(XtzktContext db, ChainConfig chain)
{
    #region static
    ...
    public async Task<Proposal> GetAsync(int epoch, string hash)
    {
        ...
        {
            proposal = await Db.Proposals.FirstOrDefaultAsync(x => x.ChainId == Chain.Id && x.Hash == hash && x.Epoch == epoch)
