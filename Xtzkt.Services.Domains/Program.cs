...
using Npgsql;
using Xtzkt.Data;
using Xtzkt.Services.Domains.Resolvers.TezosDomains;
...
builder.Configuration.AddEnvironmentVariables("XTZKT_DOMAINS_");
...
builder.Services.AddHostedService<TezosDomainsResolver>();
...
while (true)
{
    ...
    try
    {
        ...
        for (int i = 0; i < Math.Min(migrations.Count, applied.Count); i++)
        {
            ...
            {
                logger.LogError("Initialization failed: domains service and DB schema have incompatible versions. Drop the DB and restore it from the appropriate snapshot.");
                return 1;
            }
        }
        ...
        {
            logger.LogError("Initialization failed: domains service version is out of date. Update it to the newer version.");
            return 2;
        }
