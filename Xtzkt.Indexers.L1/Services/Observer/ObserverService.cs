...
using Xtzkt.Indexers.L1.Services.Observer;
        ...
        IMetrics _metrics) : IHostedService
    {
        ...
        private async Task UpdateSyncStatus()
        {
            while (!_cts.IsCancellationRequested)
            {
                try
                {
                    _appState.KnownLevel = _head.Level;
                    _appState.SyncedAt = _syncedAt.TrimMilliseconds();
                    ...
                    await db.Database.ExecuteSqlRawAsync("""
                        ...
                        SET "KnownLevel" = {0},
                            "SyncedAt" = {1}
                        WHERE "Id" = {2}
                        """, [_appState.KnownLevel, _appState.SyncedAt, _appState.Id], _cts.Token);
        ...
        private async Task AdvanceLocalBranch(CancellationToken cancellationToken)
        {
            while (_appState.Level < _head.Level && !cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    _metrics.Measure.Gauge.SetHealthValue(_appState);
                    _logger.LogInformation("Applied {level} of {total}", _appState.Level, _appState.KnownLevel);
                }
        ...
        private async Task RebaseLocalBranch(CancellationToken cancellationToken)
        {
            while (_appState.Level >= 0 && !cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    _metrics.Measure.Gauge.SetHealthValue(_appState);
                    _logger.LogInformation("Reverted to {level} of {total}", _appState.Level, _appState.KnownLevel);
                }
                catch (Exception ex)
                {
        ...
        {
            return _head.Level != _appState.KnownLevel || _syncedAt >= _appState.SyncedAt.AddSeconds(SyncStatusTtl);
        }
