...
namespace Xtzkt.Indexers.L1.Protocols.Proto18
{
    class StakingCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        ...
        public async Task Apply(L1Block block, byte[] opHash, JsonElement content)
        {
            ...
            var senderBaker = sender as L1Baker ?? Cache.Addresses.GetBaker(sender.BakerId);
            var staker = (await Cache.Addresses.GetOrCreateAsync(content.RequiredString("destination"), block) as L1User)!;
            ...
            if (operation.Status == OperationStatus.Applied)
            {
                if (result.TryGetProperty("balance_updates", out var balanceUpdates))
                {
                    ...
                    operation.BakerId = operation.Action == StakingAction.Finalize
                        ? updates.FirstOrDefault(x => x.Type == StakingUpdateType.Finalize)?.BakerId ?? sender.UnstakedBakerId ?? senderBaker?.Id
                        : updates.FirstOrDefault(x => x.Type != StakingUpdateType.Finalize)?.BakerId ?? senderBaker?.Id;
                ...
                else
                {
                    operation.Amount = 0;
                    operation.BakerId = operation.Action == StakingAction.Finalize
                        ? sender.UnstakedBakerId ?? senderBaker?.Id
                        : senderBaker?.Id;
                    operation.StakingUpdatesCount = 0;

                }

                if (Cache.Addresses.GetBaker(operation.BakerId) is L1Baker baker && baker != sender && baker != staker)
                {
                    Db.TryAttach(baker);
        ...
        public async Task Revert(L1Block block, StakingOperation operation)
        {
            var sender = (await Cache.Addresses.GetAsync(operation.SenderId) as L1User)!;
            var staker = (await Cache.Addresses.GetAsync(operation.StakerId) as L1User)!;
        ...
        async Task<List<StakingUpdate>> ParseStakingUpdates(L1Block block, StakingOperation operation, List<JsonElement> balanceUpdates)
        {
            ...
            for (int i = 0; i < balanceUpdates.Count; i += 2)
            {
                ...
                if (kind == "contract")
                {
                    ...
                    res.Add(new StakingUpdate
                    {
                        ...
                        StakerId = (await Cache.Addresses.GetExistingAsync(staker)).Id,
