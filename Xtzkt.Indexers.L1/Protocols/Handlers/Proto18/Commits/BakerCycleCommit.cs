...
namespace Xtzkt.Indexers.L1.Protocols.Proto18
{
    class BakerCycleCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
            ...
            Dictionary<int, long> selectedStakes)
        {
            ...
            var bakerCycles = snapshots.ToDictionary(x => x.AddressId, snapshot =>
            {
        ...
        protected async Task RevertCurrentRights(L1Block block)
        {
            var currentRights = await Cache.BakingRights.GetAsync(block.Level);
            var currentCycle = await Db.Cycles.SingleAsync(x => x.ChainId == block.ChainId && x.Index == block.Cycle);
        ...
        protected virtual async Task RevertNewCycle(L1Block block)
        {
        ...
        protected virtual long GetFutureAttestationRewards(L1Protocol protocol, Cycle cycle, long bakingPower)
        {
