...
namespace Xtzkt.Indexers.L1.Protocols.Proto18
{
    class StakingUpdateCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        public async Task Apply(IEnumerable<StakingUpdate> updates)
        {
            ...
            {
                var baker = Cache.Addresses.GetBaker(update.BakerId);
                Db.TryAttach(baker);
                baker.StakingUpdatesCount = (baker.StakingUpdatesCount ?? 0) + 1;
                baker.LastLevel = update.Level;
                baker.LastTimestamp = Context.Block.Timestamp;

                var staker = (L1User)await Cache.Addresses.GetAsync(update.StakerId);
                Db.TryAttach(staker);
                if (staker != baker)
                {
                    staker.StakingUpdatesCount = (staker.StakingUpdatesCount ?? 0) + 1;
                    staker.LastLevel = update.Level;
                    staker.LastTimestamp = Context.Block.Timestamp;
                }
                ...
                switch (update.Type)
                {
                    ...
                    case StakingUpdateType.Finalize:
                        ...
                        else
                        {
                            ...
                            var currentBaker = staker as L1Baker ?? Cache.Addresses.GetBaker(staker.BakerId);
        ...
        public async Task Revert(IEnumerable<StakingUpdate> updates)
        {
            foreach (var update in updates)
            {
                ...
                Cache.Chain.ReleaseStakingUpdateId();
            }
        }
        ...
        async Task UpdateUnstakeRequests(StakingUpdate update)
        {
            ...
            if (bakerUnstaked == null)
            {
                ...
                {
                    Id = Cache.Chain.NextUnstakeReqeuestId(),
            ...
            bakerUnstaked.LastTimestamp = update.Timestamp;
            stakerUnstaked.LastLevel = update.Level;
            stakerUnstaked.LastTimestamp = update.Timestamp;
        }
        ...
        async Task RevertUnstakeRequests(StakingUpdate update)
        {
            ...
            if (bakerUnstaked.UpdatesCount == 0)
            {
                ...
                Cache.Chain.ReleaseUnstakeReqeuestId();
            }
            else
            {
                ...
                bakerUnstaked.LastTimestamp = prevUpdate.Timestamp;
            }
            ...
            else
            {
                ...
                stakerUnstaked.LastTimestamp = prevUpdate.Timestamp;
            }
        }
