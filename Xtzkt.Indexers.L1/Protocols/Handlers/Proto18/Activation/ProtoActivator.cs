...
namespace Xtzkt.Indexers.L1.Protocols.Proto18
{
    partial class ProtoActivator(ProtocolHandler proto) : Proto17.ProtoActivator(proto)
    {
        protected override async Task<List<L1Address>> BootstrapAddresses(L1Protocol protocol, JToken parameters)
        {
            ...
            await new StakingUpdateCommit(Proto).Apply([..bakers
                ...
                .Select(x => new StakingUpdate
                {
                    ...
                    Level = 1,
                    Timestamp = Cache.Chain.Get().Timestamp,
                    Cycle = 0,
                    BakerId = x.Id,
                    StakerId = x.Id,
        ...
        public override List<Cycle> BootstrapCycles(L1Protocol protocol, List<L1Address> addresses, JToken parameters)
        {
            var cycles = base.BootstrapCycles(protocol, addresses, parameters);
        ...
        protected virtual long GetBlockBonusPerBlock(JsonElement issuance, L1Protocol protocol)
            => issuance.RequiredInt64("baking_reward_bonus_per_slot") * (protocol.AttestersPerBlock - protocol.ConsensusThreshold);
        
        protected virtual long GetAttestationRewardPerBlock(JsonElement issuance, L1Protocol protocol)
        ...
        protected override void SetParameters(L1Protocol protocol, JToken parameters)
        {
            ...
            protocol.BakerParametersActivationDelay = protocol.ConsensusRightsDelay;
        ...
        protected async Task RemoveDeadRefutationGames(L1Chain state)
        {
            ...
            foreach (var game in activeGames)
            {
                ...
                var rollup = await Cache.Addresses.GetAsync(game.SmartRollupId);
                Db.TryAttach(rollup);
                rollup.ActiveRefutationGamesCount--;
            }
        }

        async Task RemoveBigmapKeys(L1Chain state)
        {
            ...
            var bigmap = await Db.BigMaps
                .FirstOrDefaultAsync(x => x.ChainId == state.Id && x.Ptr == 5696);
            ...
            foreach (var key in keys)
            {
                ...
                if (((((value as MichelinePrim)!.Args![1] as MichelinePrim)!.Args![1] as MichelinePrim)!.Args![1] as MichelineInt)!.Value == BigInteger.Zero)
                {
                    ...
                    key.LastTimestamp = block.Timestamp;
                    key.Updates++;

                    bigmap.ActiveKeys--;
                    bigmap.LastLevel = block.Level;
                    bigmap.LastTimestamp = block.Timestamp;
                    bigmap.Updates++;
                }
            }
        }

        async Task MigrateBakers(L1Chain state)
        {
            UpdateBakersPower();
            ...
            var bakers = stakes
                .Where(x => x.Value > 0)
                .Select(x => Cache.Addresses.GetExistingBaker(x.Key));
            ...
            await new StakingUpdateCommit(Proto).Apply([..bakers.Select(x => new StakingUpdate
            {
                ...
                StakerId = x.Id,
                Type = StakingUpdateType.Stake,
                Amount = stakes[x.Hash]
            })]);
        }

        async Task MigrateCycles(L1Chain state)
        {
            ...
            var cycles = await Db.Cycles.Where(x => x.ChainId == state.Id && x.Index > state.Cycle).ToListAsync();
            foreach (var cycle in cycles)
            {
                var issuance = issuances.EnumerateArray().First(x => x.RequiredInt32("cycle") == cycle.Index);
