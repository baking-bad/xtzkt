...
{
    class BakingRightsCommit(ProtocolHandler protocol) : Proto01.BakingRightsCommit(protocol)
    {
            ...
            else
            {
                ...
                if (maxExistedRound < block.BlockRound)
                {
                    ...
                    foreach (var br in bakingRights.EnumerateArray().SkipWhile(x => x.RequiredInt32("priority") <= maxExistedRound))
                        sqlInsert += $@"
                            ({block.ChainId}, {block.Cycle}, {block.Level}, {Cache.Addresses.GetExistingBaker(br.RequiredString("delegate")).Id}, {(int)BakingRightType.Baking}, {(int)BakingRightStatus.Future}, {br.RequiredInt32("priority")}, null),";
