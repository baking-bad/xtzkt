...
namespace Xtzkt.Indexers.L1.Protocols.Proto16
{
    class TicketsCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        ...
        public virtual async Task Apply()
        {
            ...
            Context.Block.Events |= L1BlockEvents.Tickets;
            ...
            foreach (var (parent, opUpdates) in Updates.OrderBy(kv => kv.Key.Id))
            {
                ...
                {
                    var ticketer = (GetOrCreateAddress(ticketUpdates[0].Op, ticketIdentity.Ticketer) as L1Contract)!;
                    ...
                    if (ticketUpdates.Count == 1 || ticketUpdates.BigSum(x => x.Update.Amount) != BigInteger.Zero)
                    {
                        foreach (var (op, ticketUpdate) in ticketUpdates)
                            MintOrBurnTickets(op, ticket, ticketUpdate.Address, ticketUpdate.Amount);
                    }
                    else if (ticketUpdates.Count(x => x.Update.Amount < BigInteger.Zero) == 1)
                    {
                        ...
                        foreach (var (op, ticketUpdate) in ticketUpdates.Where(x => x.Update.Amount > BigInteger.Zero))
                            TransferTickets(ticketUpdates[0].Op, ticket, fromUpdate.Address, ticketUpdate.Address, ticketUpdate.Amount);
                    }
                    else if (ticketUpdates.Count(x => x.Update.Amount > BigInteger.Zero) == 1)
                    {
                        var (toOp, toUpdate) = ticketUpdates.First(x => x.Update.Amount > BigInteger.Zero);
                        foreach (var (op, ticketUpdate) in ticketUpdates.Where(x => x.Update.Amount < BigInteger.Zero))
                            TransferTickets(ticketUpdates[0].Op, ticket, ticketUpdate.Address, toUpdate.Address, -ticketUpdate.Amount);
                    }
                    else if (IsTransfersSequence(ticketUpdates))
                    {
                        for (int i = 0; i < ticketUpdates.Count; i += 2)
                        {
                            ...
                            if (u1.Amount < 0) // from u1 to u2
                                TransferTickets(ticketUpdates[i].Op, ticket, u1.Address, u2.Address, u2.Amount);
                            else // from u2 to u1
                                TransferTickets(ticketUpdates[i].Op, ticket, u2.Address, u1.Address, u1.Amount);
                        }
                    }
                    else
                    {
        ...
        L1Address GetOrCreateAddress(IOperation op, string addressHash)
        {
        ...
        Ticket GetOrCreateTicket(IOperation op, L1Contract ticketer, TicketIdentity ticketToken)
        {
            if (!Cache.Tickets.TryGetCached(ticketer.Id, ticketToken.RawType, ticketToken.RawContent, out var ticket))
            {
                ticket = new Ticket
                {
                    Id = op switch
                    {
                        L1TransactionOperation transaction => Cache.Chain.NextSubId(transaction),
                    ...
                    {
                        L1TransactionOperation transaction => transaction.InitiatorId ?? transaction.SenderId,
        ...
        TicketBalance GetOrCreateTicketBalance(IOperation op, Ticket ticket, L1Address address)
        {
        ...
        void TransferTickets(IOperation op, Ticket ticket, string fromAddress, string toAddress, BigInteger amount)
        {
            var from = GetOrCreateAddress(op, fromAddress);
            var fromBalance = GetOrCreateTicketBalance(op, ticket, from);
            var to = GetOrCreateAddress(op, toAddress);
            var toBalance = GetOrCreateTicketBalance(op, ticket, to);
            
            switch (op)
            {
                case L1TransactionOperation transaction:
        ...
        void MintOrBurnTickets(IOperation op, Ticket ticket, string addressHash, BigInteger amount)
        {
            var address = GetOrCreateAddress(op, addressHash);
        ...
        {
            if (!block.Events.HasFlag(L1BlockEvents.Tickets))
                return;
            ...
            foreach (var ticket in ticketsToRemove)
            {
                ...
                var contract = (L1Contract)Cache.Addresses.GetCached(ticket.TicketerId);
