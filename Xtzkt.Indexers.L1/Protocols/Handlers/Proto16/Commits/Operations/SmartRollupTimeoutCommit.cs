...
namespace Xtzkt.Indexers.L1.Protocols.Proto16
{
    class SmartRollupTimeoutCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        public virtual async Task Apply(L1Block block, byte[] opHash, JsonElement content)
        {
            ...
            var rollup = await Cache.Addresses.GetAsync(content.RequiredString("rollup"), block) as L1SmartRollup;

            var aliceId = await Cache.Addresses.GetIdOrDefaultAsync(content.Required("stakers").RequiredString("alice"));
            var bobId = await Cache.Addresses.GetIdOrDefaultAsync(content.Required("stakers").RequiredString("bob"));
