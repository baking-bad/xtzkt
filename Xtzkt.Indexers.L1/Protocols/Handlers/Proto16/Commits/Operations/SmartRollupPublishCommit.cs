...
namespace Xtzkt.Indexers.L1.Protocols.Proto16
{
    class SmartRollupPublishCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        public virtual async Task Apply(L1Block block, byte[] opHash, JsonElement content)
        {
            ...
            block.Operations |= L1Operations.SmartRollupPublish;

            sender.Counter = operation.Counter;

            commitment?.LastLevel = operation.Level;

            Cache.Chain.Get().SmartRollupPublishOpsCount++;
            #endregion
            ...
            if (operation.Status == OperationStatus.Applied)
            {
                ...
                if (commitment == null)
                {
                    ...
                    {
                        Id = Cache.Chain.NextSmartRollupCommitmentId(),
                        ChainId = block.ChainId,
                        SmartRollupId = rollup!.Id,
        ...
        public virtual async Task Revert(L1Block block, SmartRollupPublishOperation operation)
        {
            #region entities
            ...
            if (operation.Status == OperationStatus.Applied)
            {
                ...
                if (commitment!.Stakers == 1 && operation.Flags.HasFlag(SmartRollupPublishFlags.AddStaker))
                {
                    ...
                    Cache.Chain.ReleaseSmartRollupCommitmentId();
