...
namespace Xtzkt.Indexers.L1.Protocols.Proto16
{
    class SmartRollupRefuteCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        public virtual async Task Apply(L1Block block, byte[] opHash, JsonElement content)
        {
            ...
            var opponent = await Cache.Addresses.GetAsync(content.RequiredString("opponent"), block);
            ...
            Cache.Chain.Get().SmartRollupRefuteOpsCount++;
            #endregion
            ...
            if (operation.Status == OperationStatus.Applied)
            {
                if (operation.Move == RefutationMove.Start)
                {
                    ...
                    {
                        Id = Cache.Chain.NextRefutationGameId(),
                        ChainId = block.ChainId,
                        SmartRollupId = rollup.Id,
                ...
                else
                {
                    ...
                    {
                        var initiator = await Cache.Addresses.GetAsync(game.InitiatorId);
                        var initiatorBaker = Cache.Addresses.GetBaker(initiator.BakerId) ?? (initiator as L1Baker);

                        opponent = await Cache.Addresses.GetAsync(game.OpponentId);
                        var opponentBaker = Cache.Addresses.GetBaker(opponent.BakerId) ?? (opponent as L1Baker);
                        ...
                        var initiatorUpdate = updates.FirstOrDefault(x => x.RequiredString("contract") == initiator.Hash);
                        var opponentUpdate = updates.FirstOrDefault(x => x.RequiredString("contract") == opponent.Hash);
                        ...
                        {
                            if (operation.GameStatus == RefutationGameStatus.Draw || result.Required("game_status").Required("result").RequiredString("player") == initiator.Hash)
                        ...
                        {
                            if (operation.GameStatus == RefutationGameStatus.Draw || result.Required("game_status").Required("result").RequiredString("player") == opponent.Hash)
                                game.OpponentLoss = 0;
                            else
        ...
        public virtual async Task Revert(L1Block block, SmartRollupRefuteOperation operation)
        {
            #region entities
            ...
            if (operation.Status == OperationStatus.Applied)
            {
                ...
                var opponent = await Cache.Addresses.GetAsync(game.OpponentId);
                ...
                if (operation.Move == RefutationMove.Start)
                {
                    Cache.Chain.ReleaseRefutationGameId();
        ...
        async Task<SmartRollupPublishOperation> GetBondOperation(L1SmartRollup rollup, L1Address staker)
        {
        ...
        async Task<List<SmartRollupCommitment>> GetFundedCommitments(L1SmartRollup rollup, L1Address staker, SmartRollupPublishOperation bondOp)
        {
        ...
        async Task<List<SmartRollupCommitment>> GetFundedCommitments(L1SmartRollup rollup, int stakerId, long fromId, long toId)
        {
            var ids = (await Db.SmartRollupPublishOps.AsNoTracking()
                .Join(Db.SmartRollupCommitments, o => o.CommitmentId, c => c.Id, (o, c) => new { o, c })
                .Where(x =>
