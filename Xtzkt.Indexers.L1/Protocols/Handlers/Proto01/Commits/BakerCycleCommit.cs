    ...
    class BakerCycleCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
            ...
            List<BakingRight> currentRights)
        {
            ...
            if (block.Events.HasFlag(L1BlockEvents.CycleBegin))
            {
                ...
                {
                    var baker = await Cache.Addresses.GetAsync(snapshot.AddressId); // WTF: rights were given to non-baker addresses
                ...
                foreach (var br in futureBakingRights!)
                {
                    ...
                    if (!Cache.Addresses.BakerExists(br.RequiredString("delegate")))
                        continue;
                ...
                foreach (var ar in futureAttestationRights!.TakeWhile(x => x.RequiredInt32("level") < skipLevel))
                {
                    ...
                    if (!Cache.Addresses.BakerExists(ar.RequiredString("delegate")))
                        continue;
                ...
                {
                    if (!Cache.Addresses.BakerExists(ar.BakerId))
                        continue;
        ...
        #region helpers
        protected long GetFutureBlockReward(L1Protocol protocol, int cycle)
            => cycle < protocol.NoRewardCycles ? 0 : protocol.BlockReward0;

        protected long GetFutureAttestationReward(L1Protocol protocol, int cycle, int slots)
            => cycle < protocol.NoRewardCycles ? 0 : (slots * protocol.AttestationReward0);

        protected long GetBlockReward(L1Protocol protocol, int cycle)
            => cycle < protocol.NoRewardCycles ? 0 : protocol.BlockReward0;

        protected long GetAttestationReward(L1Protocol protocol, int cycle, int slots, int prevPriority)
