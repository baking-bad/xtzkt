    ...
    class VotingCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        ...
        public virtual async Task Revert(L1Block block)
        {
            ...
            if (prev.Kind != PeriodKind.Proposal || prev.ProposalsCount > 0)
            {
                var proposals = await Db.Proposals
                    .Where(x => x.ChainId == block.ChainId && x.LastPeriod >= prev.Index)
                    .ToListAsync();
            ...
            await Db.Database.ExecuteSqlRawAsync("""
                DELETE FROM "VotingPeriods" WHERE "ChainId" = {0} AND "Index" = {1};
                DELETE FROM "VotingSnapshots" WHERE "ChainId" = {0} AND "Period" = {1};
                """, block.ChainId, current.Index);
        ...
        protected async Task UpdateProposalsStatus(VotingPeriod p)
        {
            if (p.Kind == PeriodKind.Proposal)
            {
                ...
                var proposals = await Db.Proposals
                    .Where(x => x.ChainId == p.ChainId && x.Status == ProposalStatus.Active)
                    .ToListAsync();
            ...
            {
                var proposal = await Db.Proposals.FirstAsync(x => x.ChainId == p.ChainId && x.Status == ProposalStatus.Active);
        ...
        protected virtual VotingPeriod StartNextPeriod(L1Block block, L1Protocol protocol, VotingPeriod current)
        {
        ...
        protected VotingPeriod StartProposalPeriod(L1Block block, L1Protocol protocol, VotingPeriod current)
        {
            ...
            //    if (!listing.TryGetValue(Cache.Addresses.GetBaker(snapshot.BakerId).Address, out var votingPower) || snapshot.VotingPower != votingPower)
        ...
        protected VotingPeriod StartBallotPeriod(L1Block block, L1Protocol protocol, VotingPeriod current, PeriodKind kind)
        {
            var period = new VotingPeriod
            {
                ChainId = block.ChainId,
        ...
        protected VotingPeriod StartWaitingPeriod(L1Block block, L1Protocol protocol, VotingPeriod current, PeriodKind kind)
        {
            var period = new VotingPeriod
            {
                ChainId = block.ChainId,
        ...
        protected virtual int GetParticipationEma(VotingPeriod period, L1Protocol proto)
        {
        ...
        protected virtual int GetBallotQuorum(VotingPeriod period, L1Protocol proto)
        {
            var prev = Db.VotingPeriods
                .AsNoTracking()
                .Where(x => x.ChainId == period.ChainId)
