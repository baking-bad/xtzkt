...
using Xtzkt.Indexers.L1.Protocols.Proto08;

namespace Xtzkt.Indexers.L1.Protocols
{
    class Proto08Handler : ProtocolHandler
    {
        ...
        public Proto08Handler(TezosNode node, XtzktContext db, CacheService cache, QuotesService quotes, IServiceProvider services, IConfiguration config, ILogger<Proto08Handler> logger, IMetrics metrics)
                ...
                foreach (var content in operation.RequiredArray("contents", 1).EnumerateArray())
                {
                    switch (content.RequiredString("kind"))
                    {
                        ...
                        case "double_endorsement_evidence":
                            await new DoubleConsensusCommit(this).Apply(blockCommit.Block, opHash, content);
                            break;
