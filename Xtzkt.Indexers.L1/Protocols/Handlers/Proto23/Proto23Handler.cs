...
using Xtzkt.Indexers.L1.Protocols.Proto23;

namespace Xtzkt.Indexers.L1.Protocols
{
    class Proto23Handler : ProtocolHandler
    {
        public override IDiagnostics Diagnostics { get; }
        public override IHelpers Helpers { get; }
        public override IValidator Validator { get; }
        public override IRpc Rpc { get; }
        public override string VersionName => "s023_023";
        public override int VersionNumber => 23;

        public Proto23Handler(TezosNode node, XtzktContext db, CacheService cache, QuotesService quotes, IServiceProvider services, IConfiguration config, ILogger<Proto23Handler> logger, IMetrics metrics)
                ...
                foreach (var content in operation.RequiredArray("contents", 1).EnumerateArray())
                {
                    switch (content.RequiredString("kind"))
                    {
                        ...
                        case "attestations_aggregate":
                            ...
                            foreach (var (baker, slots) in attestations)
                        ...
                        case "preattestations_aggregate":
                            ...
                            foreach (var (baker, slots) in preattestations)
