...
{
    class ProposalsCommit(ProtocolHandler protocol) : Proto03.ProposalsCommit(protocol)
    {
        ...
        public override async Task Apply(L1Block block, byte[] opHash, JsonElement content)
        {
            ...
            {
                var proposalOps = (await Db.ProposalOps.Where(x => x.ChainId == block.ChainId && x.Period == period.Index).ToListAsync())
                ...
                {
                    var proposalSender = Cache.Addresses.GetBaker(proposalOp.SenderId);
                    ...
                    proposalBlock.Operations &= ~L1Operations.Proposal;
            ...
            {
                var ballotOps = (await Db.BallotOps.Where(x => x.ChainId == block.ChainId && x.Period == period.Index).ToListAsync())
                ...
                {
                    var ballotSender = Cache.Addresses.GetBaker(ballotOp.SenderId);
                    ...
                    ballotBlock.Operations &= ~L1Operations.Ballot;
            ...
            var proposals = (await Db.Proposals.Where(x => x.ChainId == block.ChainId && x.FirstPeriod == period.Index).ToListAsync())
            ...
            {
                var activeProposal = await Db.Proposals.FirstOrDefaultAsync(x => x.ChainId == block.ChainId && x.FirstPeriod != period.Index && x.LastPeriod == period.Index);
            ...
            var snapshots = await Db.VotingSnapshots.Where(x => x.ChainId == block.ChainId && x.Period == period.Index).ToListAsync();
