...
namespace Xtzkt.Indexers.L1.Protocols.Proto13
{
    class CycleCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        ...
        protected virtual Task<byte[]?> GetVdfSolution(L1Block block) => Task.FromResult<byte[]?>(null);

        protected virtual async Task<Dictionary<int, long>> GetSelectedStakes(L1Block block, L1Protocol protocol, List<SnapshotBalance> snapshots)
        {
            var attestationRewards = await Db.BakerCycles
                .AsNoTracking()
                .Where(x => x.ChainId == block.ChainId && x.Cycle == block.Cycle - 1 && x.AttestationRewardsDelegated > 0)
            ...
            return snapshots
                ...
                {
                    var baker = Cache.Addresses.GetBaker(x.AddressId);
                    ...
                    return Math.Min(x.StakingBalance, depositCap * (protocol.MaxDelegatedOverFrozenRatio + 1));
                });
        }
    }
}
