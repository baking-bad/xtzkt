...
namespace Xtzkt.Indexers.L1.Protocols.Proto05
{
    class SoftwareCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        ...
        {
            var version = $"0x{rawBlock.Required("header").RequiredString("proof_of_work_nonce")[..8]}";
            var software = await Cache.Software.GetOrCreateAsync(version, () => new Software
            {
                Id = Cache.Chain.NextSoftwareId(),
            ...
            var blockProducer = Cache.Addresses.GetBaker(block.ProducerId!.Value);
        ...
        public virtual async Task Revert(L1Block block)
        {
            ...
            // TODO: revert Baker.SoftwareId and Software.LastLevel
            // TODO: remove emptied software
        }
    }
}
