    ...
    class BakerCycleCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
            ...
            List<BakingRight> currentRights)
        {
            ...
            foreach (var rights in currentRights.GroupBy(x => x.BakerId))
            {
                ...
                if (attestationRight != null)
                {
                    ...
                    var successReward = GetAttestationReward(Context.Protocol, block.Cycle, attestationRight.Slots.Value, block.BlockRound);

                    var maxReward = bakingRights.FirstOrDefault()?.Status > BakingRightStatus.Realized
                        ? GetAttestationReward(Context.Protocol, block.Cycle, attestationRight.Slots.Value, bakingRights[0].Round!.Value)
                        : successReward;
                ...
                if (bakingRights.Count > 0)
                {
                    ...
                    {
                        bakerCycle.BlockFees += block.BakerFees;
                    }
                    else if (bakingRights[0].Status == BakingRightStatus.Missed)
                    {
                        bakerCycle.MissedBlockFees += block.BakerFees;
                    }
                    else
                    {
                        throw new Exception("Unexpected future rights");
            ...
            if (block.Events.HasFlag(L1BlockEvents.CycleBegin))
            {
                var bakerCycles = snapshots!.ToDictionary(
                    snapshot => Cache.Addresses.GetBaker(snapshot.AddressId).Hash,
                    snapshot =>
                    {
                ...
                var shiftedRights = await Db.BakingRights.AsNoTracking()
                    .Where(x => x.ChainId == block.ChainId && x.Level == futureCycle!.FirstLevel && x.Type == BakingRightType.Attestation)
                    .ToListAsync();

                foreach (var ar in shiftedRights)
                {
                    var baker = Cache.Addresses.GetBaker(ar.BakerId);

                    if (!bakerCycles.TryGetValue(baker.Hash, out var bakerCycle))
                    {
                        ...
                        var snapshottedBaker = await Proto.Rpc.GetDelegateAsync(futureCycle!.SnapshotLevel, baker.Hash);
                        ...
                        bakerCycles.Add(baker.Hash, bakerCycle);
        ...
        public virtual async Task Revert(L1Block block)
        {
            ...
            foreach (var rights in currentRights.GroupBy(x => x.BakerId))
            {
                ...
                if (bakerCycle == null)
                {
                    if (!block.Events.HasFlag(L1BlockEvents.CycleBegin) || rights.Any(x => x.Status == BakingRightStatus.Realized))
            ...
            if (block.Events.HasFlag(L1BlockEvents.CycleBegin))
            {
                await Db.Database.ExecuteSqlRawAsync("""
                    ...
                    WHERE "ChainId" = {0}
                    AND "Cycle" = {1}
                    """, block.ChainId, block.Cycle + Context.Protocol.ConsensusRightsDelay);
            }
            #endregion
        }
        ...
        protected virtual long GetBlockReward(L1Protocol protocol, int cycle, int priority, long slots)
            => cycle < protocol.NoRewardCycles ? 0 : (protocol.BlockReward0 * (8 + 2 * slots / protocol.AttestersPerBlock) / 10 / (priority + 1));

        protected virtual long GetFutureAttestationReward(L1Protocol protocol, int cycle, long slots)
            => cycle < protocol.NoRewardCycles ? 0 : (slots * protocol.AttestationReward0);

        protected virtual long GetAttestationReward(L1Protocol protocol, int cycle, int slots, int priority)
            => cycle < protocol.NoRewardCycles ? 0 : (slots * (long)(protocol.AttestationReward0 / (priority + 1.0)));
        #endregion
    }
}
