...
namespace Xtzkt.Indexers.L1.Protocols.Proto25
{
    class Validator(ProtocolHandler protocol) : IValidator
    {
        ...
        {
            Protocol = await Cache.Protocols.GetAsync(Cache.Chain.GetNextProtocol());
        ...
        void ValidateBlockHeader(JsonElement header)
        {
            ...
            if (Level != Cache.Chain.GetNextLevel())
                throw new ValidationException($"invalid block level", true);

            if (header.RequiredString("predecessor") != Cache.Chain.GetHead())
        ...
        async Task ValidateBlockMetadata(JsonElement metadata)
        {
            ...
            var period = await Cache.Periods.GetAsync(Cache.Chain.Get().VotingPeriod);
            ...
            foreach (var baker in metadata.RequiredArray("deactivated").EnumerateArray())
                if (!Cache.Addresses.BakerExists(baker.RequiredString()))
            ...
            if (balanceUpdates.Any(x => x.RequiredString("kind") == "contract" && x.RequiredString("origin") == "block" && !Cache.Addresses.BakerExists(x.RequiredString("contract"))))
                throw new ValidationException("non-existent baker in block balance updates");
            ...
            foreach (var op in metadata.RequiredArray("implicit_operations_results").EnumerateArray())
            {
                ...
                else if (kind == "origination" && Level == Protocol.FirstLevel)
                {
                    ...
                    if (!await Cache.Addresses.ExistsAsync(contract, AddressType.L1Contract))
                ...
                foreach (var op in opg.RequiredArray().EnumerateArray())
                {
                    foreach (var content in op.RequiredArray("contents").EnumerateArray())
                    {
                        switch (content.RequiredString("kind"))
                        {
                            ...
                            case "update_companion_key": await ValidateUpdateSecondaryKey(content); break;
        ...
        {
            if (content.RequiredInt32("level") != Cache.Chain.GetLevel())
                throw new ValidationException("invalid attestation level");

            if (!Cache.Addresses.BakerExists(content.Required("metadata").RequiredString("delegate")))
                throw new ValidationException("unknown attestation baker");
        }

        protected virtual void ValidateAttestationsAggregate(JsonElement content)
        {
            if (content.Required("consensus_content").RequiredInt32("level") != Cache.Chain.GetLevel())
            ...
            foreach (var c in metaCommittee)
                if (!Cache.Addresses.BakerExists(c.RequiredString("delegate")))
                    throw new ValidationException("unknown attestations aggregate baker");
        }

        protected virtual void ValidatePreattestation(JsonElement content)
        {
            if (content.RequiredInt32("level") != Cache.Chain.GetLevel() + 1)
                throw new ValidationException("invalid preattestation level");

            if (!Cache.Addresses.BakerExists(content.Required("metadata").RequiredString("delegate")))
                throw new ValidationException("unknown preattestation baker");
        }

        protected virtual void ValidatePreattestationsAggregate(JsonElement content)
        {
            if (content.Required("consensus_content").RequiredInt32("level") != Cache.Chain.GetLevel() + 1)
            ...
            foreach (var c in metaCommittee)
                if (!Cache.Addresses.BakerExists(c.RequiredString("delegate")))
                    throw new ValidationException("unknown preattestations aggregate baker");
        }
        ...
        protected virtual async Task ValidateBallot(JsonElement content)
        {
            ...
            if (Cache.Chain.Get().VotingPeriod != periodIndex)
                throw new ValidationException("invalid ballot voting period");

            var proposal = await Cache.Proposals.GetOrDefaultAsync(Cache.Chain.Get().VotingEpoch, content.RequiredString("proposal"));
            if (proposal?.Status != ProposalStatus.Active)
                throw new ValidationException("invalid ballot proposal");

            if (!Cache.Addresses.BakerExists(content.RequiredString("source")))
        ...
        protected virtual void ValidateProposal(JsonElement content)
        {
            ...
            if (Protocol.Dictator != source && !Cache.Addresses.BakerExists(source))
                throw new ValidationException("invalid proposal sender");
        }

        protected virtual async Task ValidateActivation(JsonElement content)
        {
            var address = content.RequiredString("pkh");

            if (await Cache.Addresses.ExistsAsync(address, AddressType.L1User) &&
                ((await Cache.Addresses.GetExistingAsync(address)) as L1User)!.ActivationsCount > 0)
                throw new ValidationException("address is already activated");

            if (content.Required("metadata").RequiredArray("balance_updates", 2)[1].RequiredString("contract") != address)
                throw new ValidationException("invalid activation balance updates");
        }

        protected virtual void ValidateDalEntrapmentEvidence(JsonElement content)
        {
        }
        ...
        protected virtual void ValidateDrainDelegate(JsonElement content)
        {
            ...
            if (!Cache.Addresses.BakerExists(drainedBaker))
                throw new ValidationException("unknown drained baker");
        ...
        {
            var source = content.RequiredString("source");
            var baker = content.OptionalString("delegate");

            if (!await Cache.Addresses.ExistsAsync(source))
                throw new ValidationException("unknown source address");

            if (content.Required("metadata").Required("operation_result").RequiredString("status") == "applied" && baker != null)
                if (source != baker && !Cache.Addresses.BakerExists(baker))
                    throw new ValidationException("unknown baker address");
        ...
        protected virtual void ValidateInternalDelegation(JsonElement content, string initiator)
        {
            ...
            //if (content.Required("result").RequiredString("status") == "applied" && baker != null)
            //    if (!Cache.Addresses.BakerExists(baker))
            //        throw new ValidationException("unknown baker address");
        }
