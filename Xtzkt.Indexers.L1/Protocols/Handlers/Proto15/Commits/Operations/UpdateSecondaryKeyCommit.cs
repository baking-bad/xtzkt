...
namespace Xtzkt.Indexers.L1.Protocols.Proto15
{
    class UpdateSecondaryKeyCommit(ProtocolHandler protocol) : ProtocolCommit(protocol)
    {
        public virtual async Task Apply(L1Block block, JsonElement op, JsonElement content)
        {
            ...
            {
                Cache.Chain.Get().PendingSecondaryKeys++;
            }
            #endregion
        ...
        public virtual async Task Revert(L1Block block, UpdateSecondaryKeyOperation operation)
        {
        ...
        public async Task ActivateSecondaryKeys(L1Block block)
        {
            if (!block.Events.HasFlag(L1BlockEvents.CycleBegin) || Cache.Chain.Get().PendingSecondaryKeys == 0)
                return;
        ...
        public async Task DeactivateSecondaryKeys(L1Block block)
        {
