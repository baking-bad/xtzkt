    ...
    class Sampler
    {
        ...
        public async Task Validate(ProtocolHandler proto, int block, int cycle)
        {
            ...
            for (int i = 0; i < Bakers.Length; i++)
            {
                ...
                {
                    if (PubKey.FromBase58(state.support.elements[i]).Address != proto.Cache.Addresses.GetBaker(Bakers[i]).Hash)
                        throw new Exception("Invalid sampler 'support' element");
                }
                else if (state.support.elements[i].@delegate == null)
                {
                    if (PubKey.FromBase58(state.support.elements[i].consensus_pk).Address != proto.Cache.Addresses.GetBaker(Bakers[i]).Hash)
                        throw new Exception("Invalid sampler 'support' element");
                }
                else
                {
                    if (state.support.elements[i].@delegate != proto.Cache.Addresses.GetBaker(Bakers[i]).Hash)
                        throw new Exception("Invalid sampler 'support' element");
                }
            }
